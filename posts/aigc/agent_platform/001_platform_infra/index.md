...
## 2. 架构设计
1. 当下，Agent 开发有两种实现方式:
   1. 基于 Claude Code SDK、Codex 成熟的通用 Agent，复用他们提供的 Agent 通用能力。
   2. 基于 Langgraph、Vercel 等 Agent 开发框架，自己实现 Agent Loop，灵活但是需要自己实现很多通用能力。
2. 这里我们采用第一种实现方式，并采用控制面和数据面分离的架构:
    - 控制面: 管理 System Prompt、Skills、MCP，并提供平台通用能力包括记忆、Agent 编排、SubAgent 等能力
    - 数据面: 拉取控制面提供的元数据，并基于 Agent Core 提供具体的 Agent 实现。实现上采用 ACP + Universal Event 抽象对底层 Agent Core 的依赖，从而实现多 Agent Core 灵活切换
3. 采用 SSE 实现从 Agent -> 控制面 -> UI 的消息传递。这个地方是现在考虑优化的点，原本的设计是控制面实现消息先落库再广播，从而实现消息的持久化可恢复。但是在控制面实现持久化会引入状态，导致控制面的可扩展性存在问题，计划把持久化转移到数据面，Agent 到 控制面这一层的传输不采用 SSE，Agent 直接落库到 Mysql，然后将 message 存入消息队列中。控制面到 UI 采用 SSE，直接从数据库和消息队列中读取。
4. 实现的评价:
    - 用户通过自定义提示词、SKILLS、MCP 实现具体的 Agent 逻辑，扩展性强。但是对依赖的底层模型的能力有比较高的要求
    - 可以灵活替换底层的 Agent 依赖，可以最大程度上通用 Agent 能力增强所带来的好处
    - 整体实现较重，更加适用于企业用户场景
